...
function sysCall_init()
    ...
    -- Main robot handle
    base_frame = sim.getObject("/Omni_robot")
    -- Steering handles
    ...
    if simROS then
        ...
        motor_sub = simROS.subscribe('/cmd_vel',
                                     'geometry_msgs/Twist',
...
function sysCall_sensing()
    ...
    -- https://forum.coppeliarobotics.com/viewtopic.php?t=6198
    sensor_toggles = get_sensor_config()
    timestamp = simROS.getTime()
    transformations = {}
    transformations[1] = (getTransformStamped(robotHandle, 'Omni_robot',
    -1, 'world'))
...
transformations[2] = (getTransformStamped(  wheelJoints[1],  '/./rollingJoint_fl',
    robotHandle, 'Omni_robot'))
transformations[3] = (getTransformStamped( wheelJoints[2], './rollingJoint_rl',
    robotHandle, 'Omni_robot'))

transformations[4] = (getTransformStamped(wheelJoints[3], './rollingJoint_rr',
    robotHandle, 'Omni_robot'))
transformations[5] = (getTransformStamped(wheelJoints[4], './rollingJoint_fr',
    robotHandle, 'Omni_robot'))
...
depth_info['header'] = {stamp = timestamp, frame_id = "Omni_robot"}
...
function publish_depth_camara(timestamp)
    -- Info on implemntation : https://forum.coppeliarobotics.com/viewtopic.php?t=5916
    -- and https://forum.coppeliarobotics.com/viewtopic.php?t=8638
    ...
    local res, farClippingPlane = sim.getObjectFloatParameter(depth_camara,
              ...
              depth_d = {}
              depth_d['header'] = {stamp = timestamp, frame_id = "Omni_robot"}
...
function publish_RBG_camera(timestamp)
    ...
    if (sensor_toggles["camera_rgb"]) then
        ...
        d = {}
        d['header'] = {stamp = timestamp, frame_id = "Omni_robot"}
        d['height'] = h
        d['width'] = w
        d['encoding'] = 'rgb8'
        d['is_bigendian'] = 1
...
function Motor_control_callback(vel)
    wheel_radius = 0.1
    y_vel = vel.linear.x * 2 
    x_vel = vel.linear.y * -2
    rotVel = vel.angular.z
    --wheel radius = 0.1m (note the wheels are spheres in the sim )
    --length of robot 0.25m
    --width of robot 0.16m

    --Distance from center wheel to center point lengthwise L = (0.25) + (0.1 / 2) = 0.225m
    --Distance from center wheel point with wise H = (0.16) + (0.1 / 2) = 0.18m
    
    --IK EQ for left_top -> wheel_speed = 1/ radius [Y_vel + X_vel - (L + H) * rotation]
    --IK EQ for right_top -> wheel_speed = 1/ radius [Y_vel - X_vel + (L + H) * rotation]
    --IK EQ for left_bottom -> wheel_speed = 1/ radius [Y_vel - X_vel - (L + H) * rotation]
    --IK EQ for right_bottom -> wheel_speed = 1/ radius [Y_vel + X_vel + (L + H) * rotation]

     
    --IK EQ for right wheels -> wheel_speed = 1/ radius [Y_vel + X_vel - (L + H) * rotation]
    ...
    --Setting wheel velocity for left_top wheel
    sim.setJointTargetVelocity(wheelJoints[1],(1 / 0.1 * (y_vel + x_vel -  (0.225 + 0.18) * rotVel)))
    --Setting wheel velocity for right_top wheel
    sim.setJointTargetVelocity(wheelJoints[4],(1 / 0.1 * (y_vel - x_vel +  (0.225 + 0.18) * rotVel)))
    --Setting wheel velocity for left_bottom wheel
    sim.setJointTargetVelocity(wheelJoints[2],(1 / 0.1 * (y_vel - x_vel - (0.225 + 0.18) * rotVel)))
    --Setting wheel velocity for right_bottom wheel
    sim.setJointTargetVelocity(wheelJoints[3],(1 / 0.1 * (y_vel + x_vel +  (0.225 + 0.18) * rotVel)))

...
function sysCall_init()
    ...
    --Main robot handle
    base_frame=sim.getObject("/differential_robot")
    --Motor handles
    motorLeft=sim.getObjectHandle("dif_right_motor")
    motorRight=sim.getObjectHandle("dif_left_motor")
    ...
    if simROS then
        ...
        motor_sub = simROS.subscribe('/cmd_vel', 'geometry_msgs/Twist',
        ...
        prox_pub = simROS.advertise('/cop/prox_sensors',
                                    'demo_programs/prox_sensor')
        simROS.publisherTreatUInt8ArrayAsString(prox_pub)

        -- Creating for line sensors
        line_pub = simROS.advertise('/cop/line_sensors',
                                    'demo_programs/line_sensor')
...
simROS.publisherTreatUInt8ArrayAsString(cam_into_pub)
        ...
        sim.addLog(sim.verbosity_scripterrors,
                   "ROS interface was not found. Cannot run.")
    end
...
function sysCall_sensing()
    ...
    transformations = {}
    transformations[1] = (getTransformStamped(robotHandle, 'differential_robot',
                                              -1, 'world'))
    -- Defining TF for left and right motors
    transformations[2] = (getTransformStamped(motorLeft, 'dif_left_motor',
                                              robotHandle, 'differential_robot'))
    transformations[3] = (getTransformStamped(motorRight, 'dif_right_motor',
                                              robotHandle, 'differential_robot'))
    -- Defining TF for Line sensors 
    transformations[4] = (getTransformStamped(line_left, 'line_right',
                                              robotHandle, 'differential_robot'))
    transformations[5] = (getTransformStamped(line_middle, 'line_middle',
                                              robotHandle, 'differential_robot'))
    transformations[6] = (getTransformStamped(line_right, 'line_left',
                                              robotHandle, 'differential_robot'))

    -- defining TF for camara
    transformations[7] = (getTransformStamped(RGB_camara, 'RGB_camara',
                                              robotHandle, 'differential_robot'))
    transformations[8] = (getTransformStamped(depth_camara, 'depth_camara',
                                              robotHandle, 'differential_robot'))

    -- defining TF for lidar
    transformations[9] = (getTransformStamped(Hokuyo_LIDAR, 'fastHokuyo_ROS',
                                              robotHandle, 'differential_robot'))
    -- Defining TF for prox sensors
    transformations[10] = (getTransformStamped(proxFrontLeft,
                                               'Proximity_front_left',
                                               robotHandle, 'differential_robot'))
    transformations[11] = (getTransformStamped(proxFront, 'Proximity_front',
                                               robotHandle, 'differential_robot'))
    transformations[12] = (getTransformStamped(proxFrontRight,
                                               'Proximity_front_right',
                                               robotHandle, 'differential_robot'))
    transformations[13] = (getTransformStamped(proxBack, 'Proximity_back',
                                               robotHandle, 'differential_robot'))
    transformations[14] = (getTransformStamped(proxBackLeft,
                                               'Proximity_back_left',
                                               robotHandle, 'differential_robot'))
    transformations[15] = (getTransformStamped(proxBackRight,
...
function publish_depth_camara_info(timestamp)
    ...
    depth_info['header'] = {stamp = timestamp, frame_id = "differential_robot"}
...
function publish_depth_camara(timestamp)
    ...
    local data = sim.getVisionSensorDepthBuffer(depth_camara +sim.handleflag_codedstring)

    local res, nearClippingPlane = sim.getObjectFloatParameter(depth_camara,sim.visionfloatparam_near_clipping) -- 1.00e-02
    local res, farClippingPlane = sim.getObjectFloatParameter(depth_camara,sim.visionfloatparam_far_clipping) -- 3.50e+00
    --Converting the clipping plan from 0-1 into CM (100), not this should be MM (1000), however due to issues with openCV 
    --it had to be converted to CM insted
    nearClippingPlane = nearClippingPlane * 100
    farClippingPlane = farClippingPlane * 100
    --Data = input buffer
    --sim.buffer_float = buffer type -> coppeliasim float buffer
    --multiplier = multipliy the buffer with the near clipping plan - far cliping plane
    --offset/splitSize offset applied to the buffer
    --output format
    ...
    depth_d = {}
    depth_d['header'] = {stamp = timestamp, frame_id = "differential_robot"}
...
function publish_RBG_camera(timestamp)
    ...
    if (sensor_toggles["camera_rgb"]) then
        ...
        d = {}
        d['header'] = {stamp = timestamp, frame_id = "differential_robot"}
        d['height'] = h
        d['width'] = w
        d['encoding'] = 'rgb8'
        d['is_bigendian'] = 1
...
-- #######################################
-- SUBSCRIBING
-- Callback function for the motors.S
-- takes the cmd_vel message and sets the motor velocity based on 
-- that message.
function Motor_control_callback(vel)

    -- wheel diamter = 1.0000e-01 -> 0.1m
    --  width of body -> 1.6000e-01 -> 0.16m
    -- width of wheel = 1.5000e-02 -> 0.015m
    -- Wheel separation (wheel base) =  0.175
    -- Wheel radus = 0.05
    vel_left = (vel.linear.x - vel.angular.z * 0.175 / 2) / 0.05
    vel_right = (vel.linear.x + vel.angular.z * 0.175 / 2) / 0.05
    sim.setJointTargetVelocity(motorLeft, vel_left)
    sim.setJointTargetVelocity(motorRight, vel_right)
end
-- ####################################
-- HELPER FUNCTIONS 
